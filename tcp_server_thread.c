...
int httpserv(int connection_sock)
{
...
static void *thread_process(void *connfd){
...
int start_server(int sock){
  ...
  for(;;){
    ...
    if(pthread_create(&tid,NULL,&thread_process,(void *)connection_sock)!=0){
      perror("thread ");
      continue;
    }
    printf("%d\n",tid);
